    ...
    def writeResourcesConfig(self, filename="resources.cfg", scanner: ResourceScanner =None):
        ...
        with open(file, 'w') as resCfg:
            if scanner is None:
                ...
                resCfg.write(f"# GENERATED AT: [{datetime.now()}]{linesep}")
                resCfg.write(f"# GENERATED BY: https://github.com/Bluscream/FiveMResourceScanner")
                categorized = categorizeResources(scanner.resources).items()
                resCfg.write(f"# Spawnables: {scanner.counts.spawnables} | Resources: {scanner.counts.resources} | Categories: {scanner.counts.categories} | Folders: {scanner.counts.directories}")
